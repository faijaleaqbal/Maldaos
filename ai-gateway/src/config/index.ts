...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): GatewayEnvConfig {
  ...
  return {
    providers: {
      groq: { name: "groq", apiKey: env.GROQ_API_KEY ?? "", baseUrl: env.GROQ_BASE_URL ?? "https://api.groq.com/openai/v1", model: env.GROQ_MODEL ?? "llama-3.3-70b-versatile" },
      ...
      google: { name: "google", apiKey: env.GOOGLE_AI_STUDIO_API_KEY ?? "", baseUrl: env.GOOGLE_AI_STUDIO_BASE_URL ?? "https://generativelanguage.googleapis.com/v1beta", model: env.GOOGLE_AI_STUDIO_MODEL ?? "gemini-1.5-flash" },
    },
    chain,
